...
class BaseQLearningAgent:
    ...
    class BaseQLearningAgent:
        ...
        def create_updater(self):
            ...
            # loss = tf.reduce_mean(tf.square(resp_outs - q_target))
            # Huber loss
            loss = tf.cond(err < self.delta_huber, lambda: tf.square(err) / 2,
                           lambda: self.delta_huber * (err - self.delta_huber / 2))
            up = tf.train.AdamOptimizer(self.lr)
            # tr_step = up.minimize(tf.clip_by_value(loss, -100, 100))
            tr_step = up.minimize(loss)
        ...
        def add_observations(self, env, num, greedy_eps=.25):
            ...
            for i in range(num):
                ...
                if done:
                    ...
                    if state is None:
                        state = np.array(
                            (env.get_state(), env.get_target(), env.get_target() - env.get_state())).reshape([1, -1])
            print('Buffer ready!')
    ...
    class BasePolicyAgent:
        ...
        def create_updater(self):
            ...
            loss_a = - tf.reduce_mean((tf.log(tf.clip_by_value(resp_outs, 1e-7,
                                                               1. - 1e-7)) * q_target))  # clipping to prevent log(0) and log(1) as its 0
            loss_b = - tf.reduce_mean(
                tf.log(tf.clip_by_value(self.action_output, 1e-7, 1. - 1e-7)) * self.action_output)  # same
            loss = loss_a + loss_b
    ...
    def add_observations(self, env, num, greedy_eps=.25):
        ...
        for i in range(num):
            q = self.get_q_vals(state)[0]
            a = self.act_replace_dct[np.argmax(q)]
            if np.random.rand() < greedy_eps:
                a = self.act_replace_dct[np.random.randint(self.action_dim)]
            try:
